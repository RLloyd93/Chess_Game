...
public class Bishop extends Piece {
    ...
    private final static int[] PIECE_MOVE_POSITION_COORDINATES = {-9, -7, 7, 9};
    ...
    private static boolean isFirstColumnExclusion(final int currentPosition, final int pieceOffset) {
        return BoardUtils.FIRST_COLUMN[currentPosition] && (pieceOffset == -9 || pieceOffset == 7);
    }

    private static boolean isEighthColumnExclusion(final int currentPosition, final int pieceOffset) {
        return BoardUtils.FIRST_COLUMN[currentPosition] && (pieceOffset == -7 || pieceOffset == 9);
    }
}
